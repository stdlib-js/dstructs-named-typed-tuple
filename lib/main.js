...
function factory( names, options ) { // eslint-disable-line max-lines-per-function, stdlib/jsdoc-require-throws-tags
	...
	if ( !isStringArray( names ) && !isEmptyArrayLikeObject( names ) ) {
		throw new TypeError( format( 'nullBZ', names ) );
	}
	if ( !hasDistinctElements( names ) ) {
		throw new TypeError( format( 'nullBa', names ) );
	}
	fields = names.slice();
	nfields = fields.length;
	for ( i = 0; i < nfields; i++ ) {
		if ( contains( RESERVED_PROPS, fields[ i ] ) ) {
			throw new Error( format( 'nullBb', fields[ i ] ) );
		}
	}
	opts = {
		'dtype': 'float64',
	...
	*/
	function namedtypedtuple() { // eslint-disable-line max-lines-per-function
		...
		if ( tuple.length !== nfields ) {
			throw new RangeError( format( 'nullBc', nfields, tuple.length ) );
		}
		...
		*/
		function entries() {
			...
			if ( self !== tuple ) {
				throw new TypeError( format('null1m') );
			}
		...
		*/
		function fieldOf( searchElement ) {
			...
			if ( arguments.length > 1 ) {
				i = arguments[ 0 ];
				if ( !isInteger( i ) ) {
					throw new TypeError( format( 'null7f', i ) );
				}
				if ( i >= nfields ) {
					return;
				}
		...
		*/
		function ind2key( ind ) {
			...
			if ( !isInteger( ind ) ) {
				throw new TypeError( format( 'null8A', ind ) );
			}
			if ( ind < 0 ) {
				ind = nfields + ind;
			}
		...
		*/
		function key2ind( key ) {
			...
			if ( !isString( key ) ) {
				throw new TypeError( format( 'null3F', key ) );
			}
			for ( i = 0; i < nfields; i++ ) {
		...
		function sort( compareFunction ) {
			...
			if ( arguments.length ) {
				if ( !isFunction( compareFunction ) ) {
					throw new TypeError( format( 'null3c', compareFunction ) );
				}
	...
	defineProperty( namedtypedtuple, 'from', {
		...
		'value': function from( src ) { // eslint-disable-line no-restricted-syntax
			...
			if ( this !== namedtypedtuple ) {
				throw new TypeError( format('null1n') );
			}
			nargs = arguments.length;
			if ( nargs > 1 ) {
				clbk = arguments[ 1 ];
			...
			if ( isCollection( src ) ) {
				if ( src.length !== nfields ) {
					throw new RangeError( format( 'nullBd', nfields, src.length ) );
				}
	...
	defineProperty( namedtypedtuple, 'of', {
		...
		'value': function of() { // eslint-disable-line no-restricted-syntax
			...
			if ( arguments.length !== nfields ) {
				throw new RangeError( format( 'nullBe', nfields, arguments.length ) );
			}
			args = [];
